                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="lg:col-span-2 space-y-4">
                    ...
                    {aiReport.actionable_recommendations?.recommendations?.length ? (
                      ...
                      {aiReport.detailed_insights_patterns?.insights?.length ? (
                        <div className="rounded-lg border border-slate-100 bg-white p-4 shadow-inner">
                          ...
                          <div className="space-y-2 text-sm">
                            {aiReport.detailed_insights_patterns.insights.map((insight, idx) => (
                              <div key={idx} className="rounded border border-slate-50 bg-slate-50 p-3">
                                <div className="flex items-center justify-between text-xs text-slate-600 mb-1">
                                  <span className="font-semibold text-slate-900">{insight.insight}</span>
                                  {insight.correlation_strength && <span>{insight.correlation_strength}</span>}
                                </div>
                                {insight.description && (
                                  <p className="text-slate-800 leading-relaxed">{insight.description}</p>
                                )}
                                {insight.related_metrics?.length ? (
                                  <p className="text-[11px] text-slate-600 mt-1">
                                    Liên quan: {insight.related_metrics.join(', ')}
                                  </p>
                                ) : null}
                              </div>
                            ))}
                    ...
                    <div className="space-y-4">
                      ...
                      {aiReport.risk_assessment_mitigation?.risks?.length ? (
                        <div className="rounded-lg border border-rose-100 bg-white p-4 shadow-inner text-sm space-y-2">
                          <div className="flex items-center gap-2">
                            <Activity className="h-4 w-4 text-rose-600" />
                            <h3 className="font-semibold text-gray-900">
                              {aiReport.risk_assessment_mitigation.title || 'Rủi ro & Giảm thiểu'}
                            </h3>
                          </div>
                          <div className="space-y-2">
                            {aiReport.risk_assessment_mitigation.risks.map((risk, idx) => (
                              <div key={idx} className="rounded border border-rose-50 bg-rose-50 p-3">
                                <p className="font-semibold text-rose-800">{risk.risk}</p>
                                {risk.description && <p className="text-rose-900 text-sm mt-1">{risk.description}</p>}
                                {risk.mitigation_strategy && (
                                  <p className="text-xs text-rose-700 mt-1">Giảm thiểu: {risk.mitigation_strategy}</p>
                                )}
                              </div>
                            ))}
                          </div>
                      ...
                      {aiReport.future_trend_predictions?.predictions?.length ? (
                        <div className="rounded-lg border border-sky-100 bg-white p-4 shadow-inner text-sm space-y-2">
                          <div className="flex items-center gap-2">
                            <TrendingUp className="h-4 w-4 text-sky-600" />
                            <h3 className="font-semibold text-gray-900">
                              {aiReport.future_trend_predictions.title || 'Xu hướng & Dự báo'}
                            </h3>
                          </div>
                          <div className="space-y-2">
                            {aiReport.future_trend_predictions.predictions.map((prediction, idx) => (
                              <div key={idx} className="rounded border border-sky-50 bg-sky-50 p-3">
                                <p className="font-semibold text-sky-800">{prediction.trend}</p>
                                {prediction.prediction && <p className="text-sky-900 text-sm mt-1">{prediction.prediction}</p>}
                                {prediction.drivers?.length ? (
                                  <p className="text-[11px] text-sky-700 mt-1">Động lực: {prediction.drivers.join(', ')}</p>
                                ) : null}
                              </div>
                            ))}
                          </div>
